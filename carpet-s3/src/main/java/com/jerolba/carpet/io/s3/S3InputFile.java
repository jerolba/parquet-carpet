...
public interface S3InputFile extends InputFile {
    ...
    public static class Builder {
        ...
        public Builder s3Path(String s3Path) {
            Path local = detectLocalFilePath(s3Path);
            if (local != null) {
        ...
        private static Path detectLocalFilePath(String s3Path) {
            if (s3Path == null || s3Path.isEmpty() || s3Path.startsWith("s3://") || s3Path.startsWith("s3a://")) {
                return null;
            }
            try {
                Path path;
